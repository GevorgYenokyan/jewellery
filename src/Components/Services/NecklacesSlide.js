...
function NecklacesSlide() {
  ...
  return (
    <div className="slider" ref={useSlide}>
      <div ref={useContener} className="slider_contener">
        {arr.map((el, i) => {
          return (
            <ul className="slide_img_cont" key={i}>
              <li className="slid_list">
                <p className="text">վզնոց</p>
                <img
                  className="img"
                  width={width}
                  src={`/img/necklaces/necklaces${el}.jpg`}
                  alt=""
                />
